...
async function handleLogin(request, env) {
  ...
  const token = generateToken();
  await env.USERS.put("token:" + token, key);
  return json({ ok: true, token, name: record.displayName, journalData: record.journalData, phone: record.phone || null, phoneVerified: !!record.phoneVerified });
}
...
async function handleSyncLoad(request, env) {
  ...
  if (!raw) return json({ error: "Account not found." }, 404);
  const record = JSON.parse(raw);
  return json({ ok: true, name: record.displayName, journalData: record.journalData, phone: record.phone || null, phoneVerified: !!record.phoneVerified });
}

// ---- Phone number verification (SMS via Twilio) ----
//
// A completely optional extra step, on top of the name+password login above —
// someone can add and verify a phone number from the Account page. Requires
// three secrets to be set on the Worker (see push-server/README.md):
//   wrangler secret put TWILIO_ACCOUNT_SID
//   wrangler secret put TWILIO_AUTH_TOKEN
//   wrangler secret put TWILIO_FROM_NUMBER
function isPlausiblePhoneNumber(phone) {
  // Expects "E.164" format: a plus sign, then 8-15 digits, e.g. +19175551234.
...
async function sendSms(env, toPhone, body) {
  if (!env.TWILIO_ACCOUNT_SID || !env.TWILIO_AUTH_TOKEN || !env.TWILIO_FROM_NUMBER) {
    throw new Error("SMS isn't configured on the server yet (missing Twilio secrets).");
  }
  ...
  const res = await fetch(`https://api.twilio.com/2010-04-01/Accounts/${env.TWILIO_ACCOUNT_SID}/Messages.json`, {
    ...
    body: new URLSearchParams({ To: toPhone, From: env.TWILIO_FROM_NUMBER, Body: body }),
  });
  if (!res.ok) {
    const detail = await res.text().catch(() => "");
    throw new Error("Twilio couldn't send the text (" + res.status + "). " + detail.slice(0, 200));
  }
}
...
async function handlePhoneSendCode(request, env) {
  ...
  if (!isPlausiblePhoneNumber(phone)) {
    return json({ error: "Enter your phone number in international format, starting with + and your country code (e.g. +19175551234)." }, 400);
  }
  // Simple cooldown so someone can't spam the send button (and rack up SMS costs).
  const existingRaw = await env.USERS.get("phonecode:" + token);
  if (existingRaw) {
    const existing = JSON.parse(existingRaw);
    if (existing.lastSentAt && Date.now() - existing.lastSentAt < 60000) {
      return json({ error: "Please wait a minute before requesting another code." }, 429);
    }
  }
  const code = generateSixDigitCode();
  const record = { code, phone, lastSentAt: Date.now(), expiresAt: Date.now() + 10 * 60 * 1000 };
  await env.USERS.put("phonecode:" + token, JSON.stringify(record), { expirationTtl: 60 * 15 });
  try {
    await sendSms(env, phone, `Your Quran Journal verification code is ${code}. It expires in 10 minutes.`);
  }
  catch (err) {
...
async function handlePhoneVerifyCode(request, env) {
  const body = await request.json().catch(() => null);
  const token = body && typeof body.token === "string" ? body.token : "";
  const code = body && typeof body.code === "string" ? body.code.trim() : "";
  const key = await userKeyForToken(env, token);
  if (!key) return json({ error: "Not logged in, or the session expired. Please log in again." }, 401);
  const pendingRaw = await env.USERS.get("phonecode:" + token);
  if (!pendingRaw) {
    return json({ error: "No pending code — request a new one." }, 400);
  }
  const pending = JSON.parse(pendingRaw);
  if (Date.now() > pending.expiresAt) {
    await env.USERS.delete("phonecode:" + token);
    return json({ error: "That code expired — request a new one." }, 400);
  }
  if (code !== pending.code) {
    return json({ error: "That code doesn't match. Double-check and try again." }, 400);
  }
  const raw = await env.USERS.get(key);
  if (!raw) return json({ error: "Account not found." }, 404);
  const record = JSON.parse(raw);
  ...
  await env.USERS.put(key, JSON.stringify(record));
  await env.USERS.delete("phonecode:" + token);
  return json({ ok: true, phone: record.phone });
}
...
async function handleAdminUsers(request, env) {
  ...
  do {
    ...
    for (const { name: k } of page.keys) {
      ...
      try {
        const record = JSON.parse(raw);
        users.push({ name: record.displayName, createdAt: record.createdAt, lastActiveAt: record.lastActiveAt, phone: record.phone || null, phoneVerified: !!record.phoneVerified });
...
// ---- Public widget data endpoints ----
//
// No login required — these just serve small, read-only snippets of data for
// home-screen widgets (built with the free Scriptable app on iPhone) to pull
// from. Nothing personal or account-related is exposed here.
const WIDGET_QUOTES = [
  { arabic: "فَإِنَّ مَعَ الْعُسْرِ يُسْرًا", english: "Indeed, with hardship comes ease.", source: "Surah Ash-Sharh, 94:5" },
  { arabic: "وَهُوَ مَعَكُمْ أَيْنَ مَا كُنتُمْ", english: "And He is with you wherever you are.", source: "Surah Al-Hadid, 57:4" },
  { arabic: "إِنَّ اللَّهَ مَعَ الصَّابِرِينَ", english: "Indeed, Allah is with those who are patient.", source: "Surah Al-Baqarah, 2:153" },
];
function handleWidgetVerse(request, env) {
  // Date-seeded, so the widget shows the same verse all day and changes at
  // midnight (in whatever timezone the widget's refresh happens to land in —
  // close enough for a "verse of the day" feel).
  const key = new Date().toISOString().split("T")[0];
  let hash = 0;
  for (let i = 0; i < key.length; i++) hash = (hash * 31 + key.charCodeAt(i)) >>> 0;
  const quote = WIDGET_QUOTES[hash % WIDGET_QUOTES.length];
  return json({ ok: true, ...quote });
}
async function handleWidgetPrayer(request, env) {
  const url = new URL(request.url);
  const lat = parseFloat(url.searchParams.get("lat"));
  const lng = parseFloat(url.searchParams.get("lng"));
  const method = parseInt(url.searchParams.get("method"), 10) || 3;
  const timezone = url.searchParams.get("tz") || "UTC";
  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    return json({ error: "Missing or invalid lat/lng." }, 400);
  }
  const dateInfo = localDateAndMinutes(timezone);
  let timings;
  try {
    timings = await getPrayerTimings(env, lat, lng, method, timezone, dateInfo);
  }
  catch (err) {
    return json({ error: "Could not fetch prayer times right now." }, 502);
  }
  let next = null;
  for (const name of ADHAN_PRAYER_NAMES) {
    const minutes = timeStringToMinutes(timings[name]);
    if (minutes === null) continue;
    if (minutes > dateInfo.minutes) { next = { name, time: timings[name].slice(0, 5) }; break; }
  }
  if (!next) next = { name: "Fajr", time: (timings.Fajr || "").slice(0, 5) }; // past Isha — next up is tomorrow's Fajr
  return json({ ok: true, next, timings: { Fajr: timings.Fajr, Dhuhr: timings.Dhuhr, Asr: timings.Asr, Maghrib: timings.Maghrib, Isha: timings.Isha } });
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/phone/send-code" && request.method === "POST") {
      return handlePhoneSendCode(request, env);
    }
    if (url.pathname === "/phone/verify-code" && request.method === "POST") {
      return handlePhoneVerifyCode(request, env);
    }

    // ---- Public widget data (no login needed) ----
    if (url.pathname === "/widget/verse" && request.method === "GET") {
      return handleWidgetVerse(request, env);
    }
    if (url.pathname === "/widget/prayer" && request.method === "GET") {
      return handleWidgetPrayer(request, env);
    }
